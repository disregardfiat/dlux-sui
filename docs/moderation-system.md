...
1. **Dgraph Service** (markets & safety) - Port 3003
...
#### Safety
- `GET /safety/dapp/:dappId` - Get safety status (includes `overallStatus`, `overallColor`, `safeOdds`, `confidence`, `negativeAccuracy`, `lessTested`, `totalPool` for gateway warnings)
...
```typescript
const safety = await axios.get(`${DGRAPH_SERVICE}/safety/dapp/${dappId}`);
...
4. **No Oracle Requirement**: Markets resolve purely by capital-weight; no external oracle is required for subjective labels
...
|---------|------|---------|
| Governance browser | `governance-journey-browser.spec.ts` | yes |
| Governance API | `governance.spec.ts` | no (API) |
