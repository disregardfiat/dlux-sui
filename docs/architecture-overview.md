...
1. **SUI Service** (`services/sui-service/`)
   ...
   - Authentication and SuiNS identity management
   - Text object storage
...
5. **Sandbox Service** (`frontend/sandbox/`)
   - Wildcard subdomain serving
   - dApp sandboxing
   - PWA manifest generation
   - Safety warnings and dialogs

6. **Vue Frontend** (`frontend/vue-app/`)
   - User interface
   - dApp discovery
   - Account management
   - dApp posting

7. **MCP Service** (`services/mcp-service/`)
   - AI assistant integration (read-only tools)

8. **Webhook Service** (`services/webhook-service/`)
   - GitHub webhook receiver for automated deployments
   - **Planned extension**: [Webhook Messaging & Escrow](./webhook-messaging-escrow.md) — user-registered keys, escrow-unlock messages, prorata release, dApp push/pull webhooks

9. **Ad Service** (`services/ad-service/`) (Reserved / Planned)
   - Reserved for a future standalone ad-campaign service (this repo currently uses Dgraph inventory + Walrus gateway)

## Data Layer Architecture

### SUI Blockchain + Dgraph Indexer

The platform uses a **write-to-SUI, read-from-Dgraph** architecture:

**SUI Blockchain (Write Operations):**
- SuiNS name registrations
- dApp posting and metadata updates
- ZK proof linkages and keypairs
- Prediction market creation and betting
- Governance market creation and resolution
- Financial settlement / claims (where applicable)

**Dgraph (Read Operations & Indexing):**
- Indexes all SUI blockchain data into rich graph relationships
- Provides fast queries for complex social relationships
- Contextualizes flat blockchain data into structured schemas
- Supports horizontal scaling and replication

### Blockchain Indexer

The Dgraph service runs a continuous blockchain indexer that:

1. **Monitors SUI Events**: Listens for new transactions and events
2. **Transforms Data**: Converts flat blockchain data into rich graph structures
3. **Maintains Relationships**: Builds connections between users, dApps, markets, etc.
4. **Provides Rich Queries**: Enables complex social and governance queries

```typescript
// Example: Blockchain indexer transforms flat data into relationships
// SUI Transaction: { type: 'suins_registered', name: 'alice.sui', owner: '0x...' }
// Dgraph Result: User --owns--> SuiNSName --has--> Profile --links--> ZKProofs
```

### Replication & Backup

Dgraph supports horizontal scaling through built-in replication:

```bash
# Export indexed data for replication
curl -X POST http://localhost:3003/admin/export \
  -H "Content-Type: application/json" \
  -d '{"format": "rdf", "path": "/exports/replica-001"}'

# Create backup
curl -X POST http://localhost:3003/admin/backup \
  -H "Content-Type: application/json" \
  -d '{"destination": "file:///backups/backup-2025-01-14"}'
```

### Governance Data Model

Platform parameters are stored as governance variables and controlled through prediction markets:

- **Variables**: Configurable platform parameters with annual change caps
- **Markets**: 7-day prediction markets for parameter changes
- **Bets**: Capital-weighted voting on governance proposals
- **Caps**: 10% maximum annual change to prevent platform disruption
...
interface User {
  ...
  suinsName?: string;               // Optional SuiNS name
...
### Account URLs
- **SuiNS**: `https://dlux.io/@yourname`
- **SUI Address**: `https://dlux.io/@0xabc123...`

### dApp URLs
- **Format**: `https://{permlink}.walrus.dlux.io/@{author}/{permlink}`
- **Example**: `https://my-game.walrus.dlux.io/@0xabc123.../my-game`
...
2. **Optional: Register SuiNS Name**
   - User navigates to account page (`/@{suiAddress}`)
   - Enters desired SuiNS name
   - System checks availability
   - User continues to SuiNS registration portal
   - Registration transaction is completed on SUI
...
6. **Submit**
   ...
   - dApp is now accessible at `{suins}.walrus.dlux.io/{permlink}`
...
3. **Access dApp**
   - System navigates to `{suins}.walrus.dlux.io/{permlink}`
...
1. **View Profile**
   - Navigate to `/@{suinsName}` or `/@{suiAddress}`
...
#### SuiNS Names
- `GET /suins/availability/:name` - Check if SuiNS name is available
- `GET /suins/profile/:identifier` - Get user by SuiNS name or SUI address
- `POST /suins/register-intent` - Get SuiNS registration URL
...
### Ad Service (`http://localhost:3013`) (Planned)
...
- `DGRAPH_SERVICE_URL` - Dgraph service URL (markets & governance)
...
- `DGRAPH_SERVICE_URL` - Dgraph service URL (markets & safety)
- `WALRUS_SERVICE_URL` - Walrus service URL

### Ad Service (Planned)
- `PORT` - Service port (default: 3013)
- `SUI_SERVICE_URL` - SUI service URL
- `DGRAPH_SERVICE_URL` - Dgraph service URL
- `WALRUS_SERVICE_URL` - Walrus service URL
...
- `test.dlux.io` → `localhost:3006` (Vue Frontend — test; updated on every push)
- `dlux.io` → `localhost:3007` (Vue Frontend — production; updated only via `scripts/promote-test-to-prod.sh`)
- `mcp.dlux.io` → `localhost:3009` (MCP Service)
- `webhook.dlux.io` → `localhost:3011` (Webhook Service)
- `*.walrus.dlux.io` → `localhost:3007` (Sandbox Service)
- `ads.dlux.io` → `localhost:3013` (Ad Service - Reserved / Planned)
...
2. **Market Resolution**
   - Market resolves purely by capital-weighted outcome (most capital wins)
   - Engagement metrics (CTR, conversion rate) may be displayed as informational signals but **do not** resolve the market
   - Resolution affects:
...
### Billing & Storage Funding

#### Ad Pool Revenue Flow

```
Ad Events → ZK Verification → Threshold Aggregation → Ad Pool
    ↓
Revenue Distribution (PM Pool → Creator Share → Platform Fee)
    ↓
...
1. **Term Structure**: Walrus storage purchased in 2-year terms
2. **Funding Timeline**:
   - First 50%: PM fees only
   - After 50%: Ads contribute to storage costs
   - 75-100%: "Precarious" status if underfunded
3. **Auto-Renewal**: Fully funded dApps renew automatically
4. **Expiration**: Insufficient funding leads to graceful dApp expiration

#### Billing Endpoints

- `GET /billing/overview?owner=...` - Complete monetization overview
- `POST /billing/claim` - Claim SUI payouts to wallet
- `GET /billing/storage/:dappId/:blobId` - Per-blob funding status

#### Payout Types

- **Ad Share**: Creator portion of ad revenue
- **Subscription Share**: Premium subscription revenue
- **PM Share**: Prediction market participation earnings
...
8. **Transfer**: Allow transferring SuiNS names
