...
2. **Register SuiNS Name** - User registers a SuiNS name (@username)
...
### Phase 2: SuiNS Names (Day 2)

#### 2.1 Register SuiNS Name
**User Journey:** User registers a SuiNS name (@username)

**Tests (Write First):**
```typescript
// services/sui-service/tests/suins.test.ts
describe('SuiNS', () => {
  describe('GET /suins/availability/:name', () => {
    it('should check if name is available', async () => {
      const res = await request(app)
  ...
  describe('POST /suins/register-intent', () => {
    it('should return registration url', async () => {
      const res = await request(app)
        .post('/suins/register-intent')
        .send({ name: 'testuser', suiAddress: '0x123...' })
...
**Implementation:**
- [ ] Implement SuiNS availability and profile routes
- [ ] Add SuiNS registration intent (referral support)
- [ ] Update account view to load `/suins/profile/:identifier`
- [ ] Add frontend SuiNS registration panel

**E2E Test:**
```typescript
// e2e/suins.spec.ts
test('user can check SuiNS availability', async ({ page }) => {
  await login(page)
  await page.goto('https://test.dlux.io/@0x123...')
  await page.fill('input[placeholder*="sui"]', 'testuser')
...
describe('Account View', () => {
  describe('GET /suins/profile/:identifier', () => {
    it('should return user by SuiNS name', async () => {
      await createUser({ suiAddress: '0x123...', suinsName: 'testuser' })
      ...
      expect(res.body.suinsName).toBe('testuser')
    })
...
**Implementation:**
- [ ] Implement account lookup by SuiNS/SUI address
...
export const userFactory = {
  create: (overrides?: Partial<User>) => ({
    ...
    suinsName: faker.internet.userName(),
    linkedZKPs: [],
    createdAt: new Date(),
    updatedAt: new Date(),
    ...overrides
