...
To run on **testnet** with **event streaming** (WebSocket):

1. Deploy your contracts to testnet and get the package ID.
2. Set `SUI_PACKAGE_ID` and `INDEXER_MODE=stream`:

   ```bash
   SUI_NETWORK=testnet SUI_PACKAGE_ID=0x123... INDEXER_MODE=stream npm start
   ```

3. Streaming uses `subscribeEvent`—lower CPU, events arrive as they occur.

See `infrastructure/README.md` for Docker deployment.

## Architecture

- **Indexer**: Streams or polls SUI blockchain for events and indexes objects
...
- ~~Real-time event subscription (when SUI supports it)~~ ✅ Implemented via `subscribeEvent` (INDEXER_MODE=stream)
