...
class DAppRepository {
  ...
  async findByOwnerAndPermlink(owner: string, permlink: string): Promise<SUIdApp | null> {
    return Array.from(this.dapps.values()).find(
      (dapp) => dapp.owner === owner && dapp.permlink === permlink
    ) || null;
  }
