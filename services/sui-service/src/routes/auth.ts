...
router.post('/zk-login', async (req, res) => {
  try {
    ...
    if (!suiAddress || signature === undefined || signature === null || !challengeId) {
      return res.status(400).json({ error: 'SUI address, signature, and challengeId are required' });
    }
    ...
    // Verify SUI signature against the stored challenge
    const isValid = await authService.verifyZKLogin(suiAddress, signature, challengeId, proof, provider);
    ...
    // Reverse-resolve SuiNS name (best-effort, non-blocking on failure)
    let suinsName: string | null = null;
    try {
      suinsName = await suinsService.reverseResolve(suiAddress);
    } catch {
      // non-fatal
    }
    ...
    res.json({
      ...
      user: { suiAddress, suinsName: suinsName || undefined }
    });
...
router.post('/zk-link', async (req, res) => {
  try {
    ...
    if (error instanceof Error) {
      if (error.message === 'Invalid or expired challenge') {
