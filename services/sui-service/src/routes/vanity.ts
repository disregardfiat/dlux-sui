...
router.get('/check/:vanity', async (req, res) => {
  try {
    ...
    res.json({ available, vanity });
  } catch (error: any) {
    logger.error('Error checking vanity address', error);
    res.status(400).json({ error: error.message || 'Failed to check vanity address' });
  }
});
...
router.post('/purchase', async (req, res) => {
  try {
    return res.status(501).json({
      error: 'Vanity purchases are disabled. Use SuiNS for name registration.'
    });
  } catch (error: any) {
